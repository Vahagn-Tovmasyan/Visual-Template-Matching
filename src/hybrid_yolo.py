"""Hybrid pipeline: YOLO11 pre-filter -> VLM re-ranker.

Strategy:
1. Run YOLO11 on the template to infer its class.
2. Run YOLO11 on the scene to find candidates of that class (lower threshold).
3. Use VLM to describe the template in natural language.
4. For each candidate, crop the region and send to VLM for verification.
5. Keep only VLM-confirmed detections.

This combines YOLO's ultra-fast detection with the VLM's semantic understanding.
"""
...
from .yolo import detect_yolo, DEFAULT_YOLO_MODEL

logger = logging.getLogger(__name__)


def detect_hybrid_yolo(
    ...
    """Detect template using YOLO pre-filter + VLM re-ranker.
    ...
    Args:
        ...
        yolo_threshold: YOLO detection threshold (lower = more candidates).
        model_id: Ultralytics model ID for YOLO.
        vlm_model: OpenRouter model for VLM verification.
        crop_expand: Factor to expand crop regions for context.

    Returns:
        DetectionResult with method="hybrid-yolo".
    """
    api_key = os.environ.get("OPENROUTER_API_KEY", "")
    if not api_key or api_key == "your-api-key-here":
        raise ValueError(
            "OPENROUTER_API_KEY not set. "
            "The YOLO-Hybrid method requires an API key for VLM verification. "
            "Copy .env.example to .env and add your key."
        )

    # Step 1: Get YOLO candidates (lower threshold to over-generate)
    ...
    if not yolo_result.detections:
        logger.info("Hybrid-YOLO: no YOLO candidates found")
    ...
    logger.info(
        f"Hybrid-YOLO: {len(yolo_result.detections)} YOLO candidates "
    ...
    # Step 3: Verify each YOLO candidate with VLM
    ...
    logger.info(f"Hybrid-YOLO: {len(verified)} verified detections")
